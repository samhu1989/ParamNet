...
Even when the condition in Proposition~\ref{prop:nearest} is met after optimization, there is still no guarantee for the injectivity over the continue surface. The reason is that when we turn Equ.~(\ref{equ:injective}) into Equ.~(\ref{equ:cycle_term}), we actually create a theoretical gap. 
The the constraint in Equ.~(\ref{equ:injective}) is defined over continue surface, while the term in  Equ.~(\ref{equ:cycle_term}) is defined on discrete point set. In order to fill in the gap as much as we can, we randomly sample $X$ from the predefined surface in training phase instead of minimizing the cycle regularization for any specific set of $X$. Sampling different $X$ from predefined surface is somehow (Ever since VAE \cite{VAE}, random vector as input is commonly used to encode latent variation in generation networks.) already employed by AtlasNet. For Pixel2Mesh, we add such sampling in the inverse decoding, we could have added it in original network in a similar way as AtlasNet. We choose not to do so to advocate that our cycle regularization is a general regularization technique and it can work along with surface mesh generation networks without altering the original network. Our controlled experiment validates that such random sampling is crucial for our cycle regularization technique to actually enforce injectivity.
...
Comparing to AtlasNet, Pixel2Mesh \cite{pixel2mesh} use a more complicate network structure which generate shape in a coarse-to-fine framework. In order to implement cycle regularization along with Pixel2Mesh, we need to cope with such design of network and use cycle regularization term in each level of point density.
