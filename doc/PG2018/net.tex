...
\begin{figure}[htbp]
	...
	\caption{The internal structure of \textit{K}-neighbor PointNet. \cxj{swith order of semantic network and KN PointNet.}}
...
Then the \emph{gather} operation forges the tensor of \textit{K}-neighbor points based on these indexes and input points. \cxj{what do you mean here?}
%
This operation is tensorflow \cxj{?}
...
\cxj{The main idea here is: why do you use parameters for the PointNet to predict offsets?}
...
\cxj{What is the goal of this layer?}
%
\mdf{While most objects have locally smooth surfaces, we apply a \textbf{Laplacian smoothing layer} to the deformed mesh in each block to improve the visual quality of the predicted mesh. }

\comments{
Mesh based Laplacian smooth is commonly used in mesh processing. 
The Laplacian smoothing layer applys the mesh based Laplacian smooth operation for each point in the point set as in (\ref{equ:lpl}). The mesh is constructed by triangulation on the sample points from parameter domain (i.e. the unit sphere surface).} 
...
\cxj{The results does not show self-intersections when you remove Laplacian smoothing layer.}


\subsection{Semantic Network}
\label{subsec:semnet}
\cxj{Learn parameters for PointNet in each layer or use the same weights for the five PointNets?}
%
...
We directly borrow the Chamfer distance from \cite{PSGN} as the Chamfer loss to measure the shape differences between the predicted mesh $M_p$ and the ground truth \mdf{points or mesh} $M_g$, as defined in Eq.(\ref{equ:chmf}).
%
...
\cxj{Exlain the groundtruth data first. How do you compare the predicted mesh and the groundtruth point clouds.}
...
As defined in (\ref{equ:edgereg}), $\mathcal{E}$ is the set of edges, recorded as a set of paired vertex indexes (e.g. $(i,j)$).\cxj{what's this...}
...
\cxj{Discuss how many layers, how many neighbors, kernal sizes and so on.}
...
%
To alleviate this problem, we adopt a \emph{network initialization} step to reset the parameterization network to start from a state that outputs surface without self-intersection. \cxj{confusing here.}
\cxj{For any input image, we first expect the network to produce a smaller sphere surface (radious=0.5), instead of predicting a shape to approach the ground truth.}
...
\cxj{Training parameters: batch size, learning rate, training times and so on.}
