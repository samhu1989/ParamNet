...
Even when the condition in Proposition~\ref{prop:nearest} is met after optimization, there is still no guarantee for $f$ being injective over the continue surface. The reason is that when we relax Equ.~(\ref{equ:injective}) into Equ.~(\ref{equ:cycle_term}), we relax the constraint over continue surface into term defined on discrete point set. However, our experiment result show that it is still quite effective with point density in AtlasNet and Pixel2Mesh. 
...
Comparing to AtlasNet, the Pixel2Mesh\cite{pixel2mesh} use a more complicate network structure.
